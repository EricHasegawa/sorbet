...
ExpressionPtr deepCopy(const void *avoid, const Tag tag, const void *tree, bool root) {
    ...
    switch (tag) {
        ...
        case Tag::UnresolvedConstantLit: {
            ...
            return make_expression<UnresolvedConstantLit>(exp->loc, deepCopy(avoid, exp->scope_), exp->cnst);
        }
        ...
        case Tag::ConstantLit: {
            ...
            if (exp->original) {
                originalC = make_unique<UnresolvedConstantLit>(
                    exp->original->loc, deepCopy(avoid, exp->original->scope_), exp->original->cnst);
            }
